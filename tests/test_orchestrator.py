...
def sample_request():
    ...
    return ChatRequest(
        messages=[Message(role="user", content="Test")], max_tokens=100, temperature=0.0
    )
...
async def test_orchestrator_stream_success(sample_request):
    ...
    orchestrator = Orchestrator(router=mock_router, max_operation_timeout=10.0)
...
async def test_orchestrator_stream_timeout(sample_request):
    ...
    orchestrator = Orchestrator(
        router=mock_router, max_operation_timeout=0.1  # Timeout muy corto
    )
...
async def test_orchestrator_generate_success(sample_request):
    ...
    mock_router.choose_and_generate = AsyncMock(
        return_value=ChatResponse(
            text="Generated text", provider="test_provider", provider_meta={}
        )
    )
...
async def test_orchestrator_generate_timeout(sample_request):
    ...
    orchestrator = Orchestrator(router=mock_router, max_operation_timeout=0.1)
